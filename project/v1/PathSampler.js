 ...
 *
 * Nothing here runs during animation — only at init / asset reload.
 */
 ...
 * @property {Float32Array} positions  - [x,y, x,y, ...]
 * @property {Float32Array} tangents   - [tx,ty, ...] unit length
 * @property {Float32Array} normals    - [nx,ny, ...] left-hand normals
 * @property {Float32Array} distances  - cumulative arc length per sample
...
/**
 * Fetch SVG text from a URL or File.
...
async function loadSvgText(source) {
  ...
  if (!res.ok) {
    throw new Error(`Failed to load path SVG: ${source} (${res.status})`);
  }
  return res.text();
}

/**
 * Extract the first <path> `d` attribute and viewBox from SVG markup.
...
function parseSvgPath(svgText) {
  ...
  if (!pathEl) {
    throw new Error("PathSampler: no <path> found in SVG");
  }
  const d = pathEl.getAttribute("d");
  if (!d) {
    throw new Error("PathSampler: path has empty `d`");
  }
...
/**
 * Build a temporary SVG <path> so we can call getPointAtLength / getTotalLength once.
 * @param {string} d
...
/**
 * Sample a path at equal arc-length steps and cache frame data.
 * @param {string|File} source
 * @param {{samples?: number}} [options]
 * @returns {Promise<PathSample>}
 */
export async function samplePath(source, options = {}) {
  ...
  try {
    ...
    if (!(totalLength > 0)) {
      throw new Error("PathSampler: path total length is zero");
    }

    const count = sampleCount + 1; // inclusive end
...
/**
 * Interpolate a cached sample at an absolute arc-length distance.
 * Writes into out vectors (mutates). Safe to call every frame.
 *
 * @param {PathSample} sample
 * @param {number} distance  clamped to [0, totalLength]
 * @param {{x:number,y:number}} outPos
 * @param {{x:number,y:number}} outTan
 * @param {{x:number,y:number}} outNor
 */
