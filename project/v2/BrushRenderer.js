 ...
 * BrushRenderer (v2)
 * ------------------
 * WebGL engine that keeps the Illustrator SVG brush visually identical.
 * Tessellation is an implementation detail — silhouette & proportions stay true.
 *
 * Wormholes = path start & end portals only. Brush shape never tapers.
 */
...
export const DEFAULT_SETTINGS = {
  pathSizeVw: 80,
  /** Uniform scale — height maps to this; length follows SVG aspect (no stretch). */
  brushSizeVw: 1.1,
  pageHeightVh: 300,
  pathScale: 1,
  /** Length scale vs natural SVG aspect (1 = exact proportions, no stretch). */
  stretchWidth: 1,
  /** Soft fade at path portals only (fraction of brush length) */
  wormholeSize: 0.04,
  debug: false,
  brushColor: null,
  samples: 512,
...
export class BrushRenderer {
  ...
  constructor(options) {
    ...
    this.settings = { ...DEFAULT_SETTINGS, ...(options.settings || {}), version: "v2" };
  ...
  _drawDebug() {
    ...
    ctx.fillText(`v2  progress ${(this.progress * 100).toFixed(1)}%`, 12, 20);
    ctx.fillText(
    ...
    ctx.fillText(
      `brushLen ${brushLen.toFixed(1)}  portalFade ${fade.toFixed(1)}`,
      12,
      52
    );
  }
