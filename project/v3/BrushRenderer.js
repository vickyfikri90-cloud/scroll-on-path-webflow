 ...
 * BrushRenderer (v3)
 * ------------------
 * Exact SVG tessellation (same as v2), with localized WIDTH taper at path
 * portals — never collapses vertices (no triangle-fan artifacts).
 */
...
export const DEFAULT_SETTINGS = {
  ...
  stretchWidth: 1,
  /** Portal width-taper radius as a fraction of brush length */
  wormholeSize: 0.12,
  debug: false,
  brushColor: null,
  samples: 512,
...
export class BrushRenderer {
  ...
  constructor(options) {
    ...
    this.settings = { ...DEFAULT_SETTINGS, ...(options.settings || {}), version: "v3" };
  ...
  _drawDebug() {
    ...
    ctx.fillText(`v3  progress ${(this.progress * 100).toFixed(1)}%`, 12, 20);
    ctx.fillText(
