/** @version v5 — PTF frames + live taper params */
/**
 * BrushRenderer (v5)
 * ------------------
 * Same as v4 (exact SVG, adaptive sampling, live taper), but path
 * orientation uses Parallel Transport Frames for stable normals on
 * sharp curves.
 */
...
export class BrushRenderer {
  ...
  constructor(options) {
    ...
    this.settings = { ...DEFAULT_SETTINGS, ...(options.settings || {}), version: "v5" };
  ...
  _drawDebug() {
    ...
    ctx.fillText(`v5 PTF  progress ${(this.progress * 100).toFixed(1)}%`, 12, 20);
    ctx.fillText(
